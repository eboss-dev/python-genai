...
class Client:
  ...
  To initialize the client, users can provide required arguments either directly
  or by using environment variables. Gemini API users can provide API key by
  providing input argument `api_key="your-api-key"` or by defining
  `GOOGLE_API_KEY="your-api-key"` as an environment variable

  Google Cloud users can provide inputs argument as `vertexai=false,
