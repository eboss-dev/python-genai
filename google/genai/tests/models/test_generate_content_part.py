...
def test_user_content_text(client):
  response = client.models.generate_content(
      model='gemini-1.5-flash',
      contents=types.UserContent('why is the sky blue?'),
  )
  ...
  with pytest_helper.exception_if_mldev(client, errors.ClientError):
    response = client.models.generate_content(
        ...
        contents=[
            ...
            types.ModelContent(
                'The image is about a cozy breakfast or brunch with blueberry'
                ' scones, coffee, and fresh flowers.'
            ),
            types.UserContent(
                'Is this a good environment for a family gathering?'
            ),
        ],
    )
    assert response.text
